describe('each page', () => {
  ...
  it('renders testimonials page', () => {
    ...
    cy.get('.columns picture img').as('images');

    cy.get('@images').should('have.lengthOf', expectedCount).and('be.visible');
    cy.get('@images').each((img) => {
      cy.wrap(img).should('have.attr', 'alt').and('have.length.gt', 0);
      cy.wrap(img).should('have.attr', 'title').and('have.length.gt', 0);
    });
